...
screen customnavigation():
    ...
    vbox:
        ...
        if main_menu:
            imagebutton auto "gui/mm_begin_%s.png" xpos 520 ypos 56 focus_mask True action Start()
            #кнопка ниже отвечает за экстра
            imagebutton auto "gui/mm_set_%s.png" xpos 35 ypos -105 focus_mask True action ShowMenu("load")
            imagebutton auto "gui/mm_exit_%s.png" xpos 30 ypos -708 focus_mask True action Quit()
            imagebutton auto "gui/mm_load_%s.png" xpos 335 ypos -1230 focus_mask True action ShowMenu("preferences")
            imagebutton auto "gui/mm_extra_%s.png" xpos 942 ypos -1300 focus_mask True action ShowMenu("load")
        else:
            textbutton _("Начать") action Start()
            textbutton _("История") action ShowMenu("history")
            textbutton _("Сохранить") action ShowMenu("save")
